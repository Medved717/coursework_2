...
def test_get_aeroplanes(mock_get): # В переменную внес мок, который глушит requsets.get
    ...
    mock_response_map.json.return_value = [{"boundingbox": ["48.0", "49.0", "2.0", "3.0"]}]
    ...
    # Не понимаю, почему что тут будет происходить?  Получается,
    # что людое использование json будет выводить это?
    mock_response_plane.json.return_value = {'time': 1785476770, 'states': [
        [
            '39de4f', 'TVF6306 ', 'France', 1785476769, 1785476769, 2.3485,
        47.9893, 6598.92, False, 203.59, 164.31, 11.7, None, 6941.82,
        '7635', False, 0
        ]
                                                                            ]
                                             }
